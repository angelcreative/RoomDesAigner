...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function handleMagicButtonClick() {
        const colorExtractorInput = document.getElementById("colorExtractor");
        const file = colorExtractorInput.files[0];
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                ...
                image.onload = function() {
                    ...
                    const colors = colorThief.getPalette(image, 5); // Extract the top 5 colors

                    const hexColors = colors.map(rgbToHex);
                    const promptEndy = `dense furnishings and decorations. Colors: ${hexColors.join(', ')}.`;
                    ...
                    // Call the generateImages function with the updated promptEndy
                    generateImages(null, getSelectedValues(), false, promptEndy);
                }
            }
    ...
    // The existing generateImages function should accept promptEndy as an additional parameter
