...
function checkImageStatus(fetchResultUrl) {
    ...
    .then(data => {
        if (data.status === 'processing') {
            ...
            showError(data);
            // Update ETA display on errordocument.getElementById('etaDisplay').textContent = "Error processing images.";  
        }
    })
    .catch(error => {
        console.error('Error checking image status:', error);
        showError(error);
       // Update ETA display on fetch error document.getElementById('etaDisplay').textContent = "Failed to check image status.";  
    });
}
