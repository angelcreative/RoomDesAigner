...
def transform_prompt(prompt_text):
    messages = [
       {
            "role": "system",
            "content": "You are a helpful assistant that transforms lists of values into natural language descriptions. When encountering hex color values, convert them into their corresponding color Pantone. When find a description of a human make the prompt specific to generate perfect anatomy"
        },
        {
            "role": "user",
            "content": f"Transform the following list of values into a detailed and professional natural language prompt in less than 180 words, try to mimic the Juggernaut XL AI model:\n\n{prompt_text}"
        }
    ]
