...
def transform_prompt(prompt_text):
    messages = [
       {
            "role": "system",
            "content": "You are a helpful assistant that transforms lists of values into natural language descriptions. All  hex color values must be transform to the corresponding Pantone decorations. Give priority to person, then  design style then home room or area, then the rest of values"
        },
        {
            "role": "user",
            "content": f"Transform the following list of values into a detailed and professional natural language prompt in less than 900 characters:\n\n{prompt_text}"
        }
    ]
